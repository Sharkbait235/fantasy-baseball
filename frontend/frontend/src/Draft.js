...
function Draft({ auth, players = [] }) {
  ...
  const handleSaveDraftSchedule = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/groups/${selectedGroup._id}/draft-schedule`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.token}`
  ...
  const handleStartDraft = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/drafts/${activeDraftId}/start`, { method: 'POST' });
      const data = await response.json();
  ...
  const handleEndDraft = async () => {
    ...
    try {
      setEndingDraft(true);
      const response = await fetch(`http://localhost:3001/api/drafts/${activeDraftId}/end`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${auth.token}`
        }
  ...
  const handlePickPlayer = async (player) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/drafts/${activeDraftId}/pick`, {
        method: 'POST',
  ...
  const handleJoinDraft = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3001/api/drafts/from-group', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${auth.token}`
  ...
  const fetchDraftPlayerStats = useCallback(async (playerId, season) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/players/${playerId}/official-stats?season=${season}`);
      if (!res.ok) {
  ...
  const fetchDraftModalFantasyPoints = useCallback(async (playerId, season) => {
    ...
    const requestPromise = (async () => {
      try {
        const response = await fetch(`http://localhost:3001/api/players/${safePlayerId}/fantasy-points?season=${seasonKey}`);
  ...
  const fetchDraftFantasyPoints2025 = useCallback(async (playerId) => {
    ...
    const requestPromise = (async () => {
      try {
        const response = await fetch(`http://localhost:3001/api/players/${safePlayerId}/fantasy-points?season=2025`);
